...
case "$WT_PRIVATE_ROOT" in
    "$WT_DESTINATION_PARENT"/.warptweet-openssl-fetch.*) ;;
    *)
...
WT_ARCHIVE_PATH="$WT_DOWNLOAD_DIRECTORY/$OPENSSL_ARCHIVE"
...
curl --proto '=https' --tlsv1.2 --fail --silent --show-error --location \
    --output "$WT_ARCHIVE_PATH" "$OPENSSL_SOURCE_URL"
curl --proto '=https' --tlsv1.2 --fail --silent --show-error --location \
    --output "$WT_SIGNATURE_PATH" "$OPENSSL_SIGNATURE_URL"
curl --proto '=https' --tlsv1.2 --fail --silent --show-error --location \
    --output "$WT_RELEASE_KEY_PATH" "$OPENSSL_RELEASE_KEY_URL"
...
if ! gpg --batch --homedir "$WT_GNUPG_DIRECTORY" --status-fd 1 \
    ...
    echo "OpenSSL release signature verification failed" >&2
    exit 65
fi
if ! awk -v expected="$OPENSSL_RELEASE_KEY_FINGERPRINT" \
    '$1 == "[GNUPG:]" && $2 == "VALIDSIG" && $3 == expected && $NF == expected { valid = 1 } END { exit(valid ? 0 : 1) }' \
    "$WT_SIGNATURE_STATUS_PATH"; then
    echo "OpenSSL release signature was not made by the pinned primary key" >&2
    exit 65
fi

assert_private_root_unchanged
mkdir -m 0755 "$WT_PUBLICATION_DIRECTORY"
install -m 0644 "$WT_ARCHIVE_PATH" "$WT_PUBLICATION_DIRECTORY/$OPENSSL_ARCHIVE"
install -m 0644 "$WT_SIGNATURE_PATH" "$WT_PUBLICATION_DIRECTORY/$OPENSSL_ARCHIVE.asc"
...
echo "verified $OPENSSL_ARCHIVE ($OPENSSL_SOURCE_SHA256)"
