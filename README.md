...
| --- | --- | --- |
| `warptweet.client-tunnels` | [v2](schemas/client-tunnels-v2.schema.json) | Pins the fixed installed client `ssh` digest and declares the server locator (`host` as an IP or DNS name), supervision policy, and one or more tunnels. Schema 1 is rejected. Key and trust paths are installation invariants and cannot be supplied by a manifest. Each `run --tunnel` invocation selects exactly one declaration and binds exactly `127.0.0.1`. |
