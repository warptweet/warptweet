...
- 2026-08-23: Lab pair darwin-arm64 × linux-amd64 covers the public story. Evidence `scripts/interop/work/evidence-20260823T203630Z.json` on `2af8334`: enroll, Ready, payload, live `dataplane_kex` (`mlkem768x25519-sha256` / `chacha20-poly1305@openssh.com`), composite auth, tampered invite fail-closed, second grant, two independent routes, session/exec refused, down/up/rotate/revoke. Privilege probe PASS. Server reboot reconnect was proven earlier on this host. Homebrew CTA stays dark (`ValidateIndex` still needs darwin-amd64 and linux-arm64; remaining WP8 cells `not_run`). No Compose/Postgres on the lab host. Darwin evidence host is macOS 26.5.2; Mach-Os remain `minos 13.0`. No new RC. Website polish can start against the dark CTA. Known leftover: `up` after rotate still fails readiness.
