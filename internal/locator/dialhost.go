// Package locator canonicalizes published WarpTweet dial hosts and resolves
// them once into a bounded candidate plan.
package locator
...
func IPDialHost(addr netip.Addr) DialHost {
	return DialHost{IP: canonicalAddr(addr)}
}
...
func (host DialHost) Canonical() (string, error) {
	...
	case hasIP:
		...
		return canonicalAddr(host.IP).String(), nil
	case hasName:
...
func canonicalAddr(address netip.Addr) netip.Addr {
